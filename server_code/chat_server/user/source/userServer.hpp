...
#include "dataES.hpp"    // es数据管理客户端封装
#include "dataRedis.hpp" // redis数据管理客户端封装
#include "mysqlUser.hpp" // mysql数据管理客户端封装
#include "etcd.hpp"       // 服务注册模块封装
#include "logger.hpp"     // 日志模块封装
#include "util.hpp"      // 基础工具接口
// #include "verifyCode.hpp"        // 短信平台SDK模块封装
...
namespace my_chat
{
  class UserServiceImpl : public my_chat::UserService
  {
  public:
    ...
    virtual void UserRegister(::google::protobuf::RpcController *controller,
                              const ::my_chat::UserRegisterReq *request,
                              ::my_chat::UserRegisterRsp *response,
    ...
    virtual void UserLogin(::google::protobuf::RpcController *controller,
                           const ::my_chat::UserLoginReq *request,
                           ::my_chat::UserLoginRsp *response,
                           ::google::protobuf::Closure *done)
    {
      LOG_DEBUG("收到用户登录请求！");
      brpc::ClosureGuard rpc_guard(done);
    ...
    //                                 const ::my_chat::PhoneVerifyCodeReq *request,
    //                                 ::my_chat::PhoneVerifyCodeRsp *response,
    //                                 ::google::protobuf::Closure *done)
    // {
    ...
    //                            const ::my_chat::PhoneRegisterReq *request,
    //                            ::my_chat::PhoneRegisterRsp *response,
    //                            ::google::protobuf::Closure *done)
    // {
    //   LOG_DEBUG("收到手机号注册请求！");
    //   brpc::ClosureGuard rpc_guard(done);
    ...
    //                         const ::my_chat::PhoneLoginReq *request,
    //                         ::my_chat::PhoneLoginRsp *response,
    //                         ::google::protobuf::Closure *done)
    // {
    //   LOG_DEBUG("收到手机号登录请求！");
    //   brpc::ClosureGuard rpc_guard(done);
    ...
    virtual void GetUserInfo(::google::protobuf::RpcController *controller,
                             const ::my_chat::GetUserInfoReq *request,
                             ::my_chat::GetUserInfoRsp *response,
                             ::google::protobuf::Closure *done)
    {
      ...
      user_info->set_nick_name(user->nickname());
      user_info->set_person_signature(user->description());
      user_info->set_phone_number(user->phone());

      if (!user->head_portrait_id().empty())
      {
        ...
        my_chat::GetSingleFileReq req;
        my_chat::GetSingleFileRsp rsp;
        req.set_request_id(request->request_id());
        req.set_file_id(user->head_portrait_id());
        ...
        user_info->set_head_portrait(rsp.file_data().file_content());
      }
      // 4. 组织响应，返回用户信息
      response->set_request_id(request->request_id());
      response->set_success(true);
    ...
    virtual void GetMultiUserInfo(::google::protobuf::RpcController *controller,
                                  const ::my_chat::GetMultiUserInfoReq *request,
                                  ::my_chat::GetMultiUserInfoRsp *response,
                                  ::google::protobuf::Closure *done)
    {
      ...
      my_chat::FileService_Stub stub(channel.get());
      my_chat::GetMultiFileReq req;
      my_chat::GetMultiFileRsp rsp;
      req.set_request_id(request->request_id());
      for (auto &user : users)
      {
        if (user.head_portrait_id().empty())
          continue;
        req.add_file_id_list(user.head_portrait_id());
      }
      brpc::Controller cntl;
      ...
      for (auto &user : users)
      {
        ...
        user_info.set_person_signature(user.description());
        user_info.set_phone_number(user.phone());
        user_info.set_head_portrait((*file_map)[user.head_portrait_id()].file_content());
        (*user_map)[user_info.user_id()] = user_info;
      }
      response->set_request_id(request->request_id());
      response->set_success(true);
    }
    virtual void SetUserHeadPortrait(::google::protobuf::RpcController *controller,
                               const ::my_chat::SetUserHeadPortraitReq *request,
                               ::my_chat::SetUserHeadPortraitRsp *response,
                               ::google::protobuf::Closure *done)
    {
      ...
      my_chat::FileService_Stub stub(channel.get());
      my_chat::PutSingleFileReq req;
      my_chat::PutSingleFileRsp rsp;
      req.set_request_id(request->request_id());
      req.mutable_file_data()->set_file_name("");
      req.mutable_file_data()->set_file_size(request->head_portrait().size());
      req.mutable_file_data()->set_file_content(request->head_portrait());
      brpc::Controller cntl;
      ...
      std::string head_portrait_id = rsp.file_info().file_id();
      // 4. 将返回的头像文件 ID 更新到数据库中
      user->head_portrait_id(head_portrait_id);
      bool ret = _mysql_user->update(user);
      if (ret == false)
      {
        LOG_ERROR("{} - 更新数据库用户头像ID失败 ：{}！", request->request_id(), head_portrait_id);
        return err_response(request->request_id(), "更新数据库用户头像ID失败!");
      }
      // 5. 更新 ES 服务器中用户信息
      ret = _es_user->appendData(user->user_id(), user->phone(),
                                 user->nickname(), user->description(), user->head_portrait_id());
      if (ret == false)
      {
        LOG_ERROR("{} - 更新搜索引擎用户头像ID失败 ：{}！", request->request_id(), head_portrait_id);
    ...
    virtual void SetUserNickname(::google::protobuf::RpcController *controller,
                                 const ::my_chat::SetUserNicknameReq *request,
                                 ::my_chat::SetUserNicknameRsp *response,
                                 ::google::protobuf::Closure *done)
    {
    ...
    virtual void SetUserSignature(::google::protobuf::RpcController *controller,
                                    const ::my_chat::SetUserSignatureReq *request,
                                    ::my_chat::SetUserSignatureRsp *response,
                                    ::google::protobuf::Closure *done)
    {
      ...
      // 1. 从请求中取出用户 ID 与新的昵称
      std::string uid = request->user_id();
      std::string new_description = request->signature();
      // 3. 从数据库通过用户 ID 进行用户信息查询，判断用户是否存在
      auto user = _mysql_user->select_by_id(uid);
      if (!user)
      {
    ...
    //                                 const ::my_chat::SetUserPhoneNumberReq *request,
    //                                 ::my_chat::SetUserPhoneNumberRsp *response,
    //                                 ::google::protobuf::Closure *done)
    // {
