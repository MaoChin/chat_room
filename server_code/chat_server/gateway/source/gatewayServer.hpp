#include "dataRedis.hpp" // redis数据管理客户端封装
...
#define SET_USER_HEAD_PORTRAIT "/service/user/set_head_portrait"
#define SET_USER_NICKNAME "/service/user/set_nickname"
#define SET_USER_DESC "/service/user/set_signature"
...
#define MSG_GET_RANGE "/service/message_storage/search_history_by_time"
#define MSG_GET_RECENT "/service/message_storage/get_recent"
#define MSG_KEY_SEARCH "/service/message_storage/search_history_by_search_key"
...
#define VOICE_RECOGNITION "/service/voice/recognition"
  ...
  public:
    ...
    GatewayServer(
        ...
        const std::string voice_service_name,
        ...
        : _redis_session(std::make_shared<Session>(redis_client)),
          ...
          _voice_service_name(voice_service_name),
          ...
          _connections(std::make_shared<Connection>())
    {
      ...
      _http_server.Post(SET_USER_HEAD_PORTRAIT, (httplib::Server::Handler)std::bind(&GatewayServer::SetUserHeadPortrait, this, std::placeholders::_1, std::placeholders::_2));
      ...
      _http_server.Post(SET_USER_DESC, (httplib::Server::Handler)std::bind(&GatewayServer::SetUserSignature, this, std::placeholders::_1, std::placeholders::_2));
      ...
      _http_server.Post(MSG_GET_RANGE, (httplib::Server::Handler)std::bind(&GatewayServer::MsgSearchByTime, this, std::placeholders::_1, std::placeholders::_2));
      ...
      _http_server.Post(MSG_KEY_SEARCH, (httplib::Server::Handler)std::bind(&GatewayServer::MsgSearchBySearchKey, this, std::placeholders::_1, std::placeholders::_2));
      ...
      _http_server.Post(VOICE_RECOGNITION, (httplib::Server::Handler)std::bind(&GatewayServer::VoiceRecognition, this, std::placeholders::_1, std::placeholders::_2));
  ...
  private:
    ...
    void SetUserHeadPortrait(const httplib::Request &request, httplib::Response &response)
    {
      // 1. 取出http请求正文，将正文进行反序列化
      SetUserHeadPortraitReq req;
      SetUserHeadPortraitRsp rsp;
      auto err_response = [&req, &rsp, &response](const std::string &errmsg) -> void
      {
        rsp.set_success(false);
        rsp.set_errmsg(errmsg);
    ...
    void SetUserSignature(const httplib::Request &request, httplib::Response &response)
    {
      // 1. 取出http请求正文，将正文进行反序列化
      SetUserSignatureReq req;
      SetUserSignatureRsp rsp;
      auto err_response = [&req, &rsp, &response](const std::string &errmsg) -> void
      {
        rsp.set_success(false);
        rsp.set_errmsg(errmsg);
    ...
    void FriendAdd(const httplib::Request &request, httplib::Response &response)
    {
      ...
      auto conn = _connections->connection(req.friend_id());
      if (rsp.success() && conn)
      {
        LOG_DEBUG("找到被申请人 {} 长连接，对其进行好友申请通知", req.friend_id());
    ...
    void FriendRemove(const httplib::Request &request, httplib::Response &response)
    {
      ...
      if (rsp.success() && conn)
      {
        LOG_ERROR("对被删除人 {} 进行好友删除通知！", req.friend_id());
        NotifyMessage notify;
        notify.set_notify_type(NotifyType::FRIEND_REMOVE_NOTIFY);
        notify.mutable_friend_delete()->set_user_id(*uid);
        conn->send(notify.SerializeAsString(), websocketpp::frame::opcode::value::binary);
      }
      // 5. 向客户端进行响应
      response.set_content(rsp.SerializeAsString(), "application/x-protbuf");
    }
    void UserSearch(const httplib::Request &request, httplib::Response &response)
    {
      UserSearchReq req;
      UserSearchRsp rsp;
      auto err_response = [&req, &rsp, &response](const std::string &errmsg) -> void
    ...
    void MsgSearchByTime(const httplib::Request &request, httplib::Response &response)
    {
      MsgSearchByTimeReq req;
      MsgSearchByTimeRsp rsp;
      auto err_response = [&req, &rsp, &response](const std::string &errmsg) -> void
      {
        rsp.set_success(false);
        rsp.set_errmsg(errmsg);
      ...
      // 2. 客户端身份识别与鉴权
      std::string ssid = req.login_session_id();
      auto uid = _redis_session->uid(ssid);
      if (!uid)
      {
        LOG_ERROR("{} 获取登录会话关联用户信息失败！", ssid);
    ...
    void MsgSearchBySearchKey(const httplib::Request &request, httplib::Response &response)
    {
      MsgSearchBySearchKeyReq req;
      MsgSearchBySearchKeyRsp rsp;
      auto err_response = [&req, &rsp, &response](const std::string &errmsg) -> void
      {
        rsp.set_success(false);
        rsp.set_errmsg(errmsg);
    ...
    void GetSingleFile(const httplib::Request &request, httplib::Response &response)
    {
      ...
      my_chat::FileService_Stub stub(channel.get());
      brpc::Controller cntl;
    ...
    void VoiceRecognition(const httplib::Request &request, httplib::Response &response)
    {
      LOG_DEBUG("收到语音转文字请求！");
      VoiceRecognitionReq req;
      VoiceRecognitionRsp rsp;
      auto err_response = [&req, &rsp, &response](const std::string &errmsg) -> void
      {
        rsp.set_success(false);
        rsp.set_errmsg(errmsg);
      ...
      req.set_user_id(*uid);
      // 3. 将请求转发给好友子服务进行业务处理
      auto channel = _mm_channels->choose(_voice_service_name);
      if (!channel)
      {
        LOG_ERROR("{} 未找到可提供业务处理的用户子服务节点！", req.request_id());
        return err_response("未找到可提供业务处理的用户子服务节点！");
      }
      my_chat::VoiceService_Stub stub(channel.get());
      brpc::Controller cntl;
      stub.VoiceRecognition(&cntl, &req, &rsp, nullptr);
      if (cntl.Failed())
      {
  ...
  private:
    ...
    std::string _voice_service_name;
  ...
  public:
                               ...
                               const std::string &transmit_service_name)
    {
      ...
      _mm_channels->declared(voice_service_name);
    ...
    GatewayServer::ptr build()
    {
      ...
      GatewayServer::ptr server = std::make_shared<GatewayServer>(
          ...
          _voice_service_name, _message_service_name,
