...
// voiceServer的测试客户端
// 1. 进行服务发现--发现voiceServer的服务器节点地址信息并实例化的通信信道
// 2. 读取语音文件数据
// 3. 发起语音识别RPC调用
...
DEFINE_string(voice_service, "/service/voice_service", "服务监控根目录");
...
int main(int argc, char *argv[])
{
    ...
    sm->declared(FLAGS_voice_service);
    ...
    auto channel = sm->choose(FLAGS_voice_service);
    if (!channel) {
    ...
    my_chat::VoiceRecognitionReq req;
    req.set_voice_content(file_content);
    req.set_request_id("111111");

    brpc::Controller *cntl = new brpc::Controller();
    my_chat::VoiceRecognitionRsp *rsp = new my_chat::VoiceRecognitionRsp();
