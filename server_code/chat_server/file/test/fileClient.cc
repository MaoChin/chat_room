...
my_chat::ServiceChannel::ChannelPtr channel;
std::string single_file_id;

TEST(put_test, single_file) {
    //1. 读取当前目录下的指定文件数据
    std::string body;
    ASSERT_TRUE(my_chat::readFile("./Makefile", body));
    ...
    my_chat::PutSingleFileRsp *rsp = new my_chat::PutSingleFileRsp();
...
TEST(get_test, single_file) {
    ...
    brpc::Controller *cntl = new brpc::Controller();
    rsp = new my_chat::GetSingleFileRsp();
    stub.GetSingleFile(cntl, &req, rsp, nullptr);
    ASSERT_FALSE(cntl->Failed());
    ASSERT_TRUE(rsp->success());
    //将文件数据，存储到文件中
    ASSERT_EQ(single_file_id, rsp->file_data().file_id());
    my_chat::writeFile("make_file_download", rsp->file_data().file_content());
}
...
TEST(put_test, multi_file) {
    //1. 读取当前目录下的指定文件数据
    std::string body1;
    ASSERT_TRUE(my_chat::readFile("./base.pb.h", body1));
    std::string body2;
    ASSERT_TRUE(my_chat::readFile("./file.pb.h", body2));
    //2. 实例化rpc调用客户端对象，发起rpc调用
    my_chat::FileService_Stub stub(channel.get());

    my_chat::PutMultiFileReq req;
    ...
    brpc::Controller *cntl = new brpc::Controller();
    my_chat::PutMultiFileRsp *rsp = new my_chat::PutMultiFileRsp();
...
TEST(get_test, multi_file) {
    ...
    brpc::Controller *cntl = new brpc::Controller();
    rsp = new my_chat::GetMultiFileRsp();
    ...
    my_chat::writeFile("base_download_file1",file_data1.file_content());
    auto file_data2 = map[multi_file_id[1]];
    my_chat::writeFile("file_download_file2", file_data2.file_content());
}
...
int main(int argc, char *argv[])
{
    ...
    //1. 先构造Rpc信道管理对象
    auto sm = std::make_shared<my_chat::ServiceManager>();
