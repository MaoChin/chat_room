...
#include "mysqlMessage.hpp" // mysql数据管理客户端封装
...
namespace my_chat
{
  class MessageServiceImpl : public my_chat::MsgStorageService
  {
  public:
    MessageServiceImpl(
                               ...
                               ::google::protobuf::Closure *done)
    {
      ...
      for (const auto &msg : msg_lists)
      {
        ...
        case MessageType::TEXT:
          message_info->mutable_message()->set_message_type(MessageType::TEXT);
          message_info->mutable_message()->mutable_text_message()->set_text_content(msg.content());
          break;
        ...
        case MessageType::VOICE:
          message_info->mutable_message()->set_message_type(MessageType::VOICE);
          message_info->mutable_message()->mutable_voice_message()->set_file_id(msg.file_id());
          message_info->mutable_message()->mutable_voice_message()->set_voice_contents(file_data_lists[msg.file_id()]);
          break;
        default:
          LOG_ERROR("消息类型错误！！");
          return;
    ...
    virtual void GetRecentMsg(::google::protobuf::RpcController *controller,
                              const ::my_chat::GetRecentMsgReq *request,
                              ::my_chat::GetRecentMsgRsp *response,
                              ::google::protobuf::Closure *done)
    {
      brpc::ClosureGuard rpc_guard(done);
      auto err_response = [this, response](const std::string &rid,
    ...
    void onMessage(const char *body, size_t sz)
    {
      ...
      case MessageType::TEXT:
        content = message.message().text_message().text_content();
      ...
      case MessageType::VOICE:
      {
        const auto &msg = message.message().voice_message();
        ret = _PutFile("", msg.voice_contents(), msg.voice_contents().size(), file_id);
        if (ret == false)
        {
      ...
      my_chat::Message msg(message.message_id(),
                           message.chat_session_id(),
                           message.sender().user_id(),
